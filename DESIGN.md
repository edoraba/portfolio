...
- Page transitions: field persists, and a typed navigation is a clip mask, 1000ms on cubic-bezier(0.75, 0, 0.1, 1): the page being left keeps moving the way the reader was going and thins to nothing, the one arriving is cut in over it from the edge it came from and carries its own ground. One movement and only one: the shared-element morph of the project title is gone, because the mask that opens the arriving page also clips the group flying into it, so the title crossed the screen cut in half and then completed itself. Back and forward crossfade in 200ms.
